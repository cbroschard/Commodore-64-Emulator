...
po::options_description get_config_file_options()
{
    ...
    desc.add_options()
        ...
        ("c64.CHAR.ROM", po::value<std::string>()->required(), "Full path and filename of the C64 CHAR ROM to load")
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        // Startup the system
        const bool boot = c64.boot();
        if (!boot)
        {
            std::cerr << "Problem booting" << std::endl;
            return 1;
        }
...
JoystickMapping parseJoystickConfig(const std::string& config)
{
    ...
    if (jm.up == SDL_SCANCODE_UNKNOWN ||
        jm.down == SDL_SCANCODE_UNKNOWN ||
        jm.left == SDL_SCANCODE_UNKNOWN ||
        jm.right == SDL_SCANCODE_UNKNOWN ||
