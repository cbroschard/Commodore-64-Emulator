...
void Vic::evaluateRasterIRQCompare(const char* reason)
{
    ...
    // Preserve debug snapshot information.
    ...
    // Raster IRQ is generated by the comparator entering
    // the matching state.
    //
    // Acknowledge through $D019 does NOT alter comparator
    // state, so acknowledging while the raster still matches
    // cannot retrigger the IRQ.
    //
    // Retargeting away from the current raster and then back
    // can, however, create a genuine new 0 -> 1 transition.
    if (matchNow && !matchedBefore)
        raiseVicIRQSource(0x01);

    // Comparator level is independent of the IRQ status latch.
